...
<t:wrapper>
    ...
    <jsp:body>
        <div class="main">
                <h2>Funds</h2><br>
                <c:forEach items="${allFU}" var="item">
                    <div class="card past">
                            <div class="content">
                                <h4>${item.get("amc_fund_house")}</h4>
                                <p>Average Buy Price : ${item.get("average_buy_price")}</p>
                                <p>Quantity : ${item.get("quantity")}</p>
                                <form action = "/customer/editfund" method = "GET">
                                    <input type="hidden" name = "type" value = '${item.get("amc_fund_house")}'/>
                                    <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                                    <button class = "edit" type="submit">Edit</button>
                                </form>
                                <form action = "/customer/deletefund" method = "POST">
                                    <input type="hidden" name = "type" value = '${item.get("amc_fund_house")}'/>
                                    <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
